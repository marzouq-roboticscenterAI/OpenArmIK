...
- The installed C ABI is permanently query-only. The private integration layer
  ...
  They cannot cross instances or replay. Every Linux SocketCAN backend
  categorically refuses issuance regardless of driver, name, or sysfs path;
  only an injected simulator/test backend may issue. This keeps physical enable,
  motion, zero, write, clear, and save unavailable in Stage A.
...
- The codec validation object is internalized into the installed static archive,
  and an exported CMake package carries its C++/Threads link contract. A clean
  install-tree project compiles the strict C11 consumer, links only the installed
  `OpenArm::openarm_transport` target, and runs it.
...
```text
cmake -S transport -B /tmp/openarmik-socketcan-release3 \
  -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON
cmake --build /tmp/openarmik-socketcan-release3 --parallel
ctest --test-dir /tmp/openarmik-socketcan-release3 --output-on-failure
4/4 tests passed, including external installed C11 consumer
```

Debug ASan + UBSan build with leak detection and halt-on-error:

```text
cmake -S transport -B /tmp/openarmik-socketcan-sanitize3 \
  -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON \
  -DOA_TRANSPORT_ENABLE_SANITIZERS=ON
cmake --build /tmp/openarmik-socketcan-sanitize3 --parallel
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 \
UBSAN_OPTIONS=halt_on_error=1 \
ctest --test-dir /tmp/openarmik-socketcan-sanitize3 --output-on-failure
3/3 tests passed
```
...
```text
cmake -S transport -B /tmp/openarmik-socketcan-tsan3 \
  -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON \
  -DOA_TRANSPORT_ENABLE_THREAD_SANITIZER=ON
cmake --build /tmp/openarmik-socketcan-tsan3 --parallel
TSAN_OPTIONS=halt_on_error=1 \
ctest --test-dir /tmp/openarmik-socketcan-tsan3 --output-on-failure
3/3 tests passed
```
