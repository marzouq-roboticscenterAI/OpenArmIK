...
<section class="viewer"><h2>3D view</h2><div class="caption">The 3D view is a separate <strong>RViz</strong> window opened by this launcher, showing the measured pose of the pinned OpenArm v1.0 model. It is loaded with a panel-free layout, so only the render view is shown; run <code>scripts/launch_rviz.sh</code> for the full RViz engineering layout with the Displays and Views panels. Start the portal with <code>--no-rviz</code> to suppress the window.</div><div class="caption">RViz renders the robot model only. It is not collision checking and its freshness is never used as control feedback.</div></section></main>
